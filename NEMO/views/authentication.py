...
from ldap3 import Tls, Server, Connection, SIMPLE, NONE
        ...
        for server in settings.LDAP_SERVERS:
            try:
                tls = Tls(
                    ...
                    ca_certs_file=server['certificate']
                )
                s = Server(
                    server['url'],
                    use_ssl=True,
                ...
                c = Connection(
                    ...
                    user='{}={},{}'.format(server['attribute'],username, server['base']),
                    password=password,
                    auto_bind="NONE",
                    version=3,
...
def login_user(request):
    ...
    if 'NEMO.views.db_authentication.DbAuthenticationBackend' in settings.AUTHENTICATION_BACKENDS:
        ...
        dictionary['reset_password_link'] = ""
